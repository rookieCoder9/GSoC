...
export default {
  ...
  activeFiles: (state, getters) => {
    ...
    const extensions = ['png', 'jpeg', '', 'jpg', 'gif', 'bmp', 'svg', 'ico', 'heic']
    if (store.state.imgmode.active) {
      files = files.filter((file) => extensions.includes(file.extension))
    }
    console.log(store.state.imgmode.active)
    console.log(files)
    return files
  },
